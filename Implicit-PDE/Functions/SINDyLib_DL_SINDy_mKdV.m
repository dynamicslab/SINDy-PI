...
function [Data,Sym_Struct]=SINDyLib_DL_SINDy_mKdV(U,Ut,Utt,Ux,Uxx,Uxxx)
...
Org_Data=[U Ut Ux Uxx Uxxx];
Org_Sym=[u ut ux uxx uxxx];
...
for i=1:size(Org_Data,2)
    Index=Index+1;
    Data(:,Index)=Org_Data(:,i).^2;
    Sym_Struct{1,Index}=Org_Sym(1,i).^2;
end
...
for i=3:size(Org_Data,2)
    Index=Index+1;
    Data(:,Index)=Org_Data(:,1).^2.*Org_Data(:,i);
    Sym_Struct{1,Index}=Org_Sym(1,1)^2*Org_Sym(1,i);
end
